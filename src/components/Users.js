...
function Users() {
    ...
    const totalPages = Math.ceil(data.length / recordPerPage) || 1;
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {records.map((item, id) => (
                    <div className='bg-slate-400 text-white p-4 rounded-lg border border-gray-300' key={id}>
                        <h4 className="text-xl font-bold mb-2">Name: {item.name}</h4>
                        <p className="text-lg mb-2">Username: {item.username}</p>
                        <p className="text-lg mb-2">City: {item.address.city}</p>
